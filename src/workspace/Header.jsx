...
function Header({ user, filename, collaborators }) {
  return(
    <div style={{width: "100%", height: HEADER_HEIGHT, background: COLORS.WHITE}}>
        <img src={logo} alt="logo" style={{display: "inline-block", height: 55, width: "auto", float: "left", marginLeft: 25, marginTop: 7}} />
        ...
        <Button style={{...style.BUTTON_STYLE, float: "right", height: 37, width: 118, marginTop: 19, marginRight: 35}}>SHARE CODE</Button>
        <div style={{float: "right", marginTop: 12, marginRight: 50}}>
          {
            collaborators.map((item, i) => 
              <div key={JSON.stringify(item)} style={{display: "inline-block", position: "relative", left: 10 * (collaborators.length - i - 1), zIndex: i}}><Avatar user={item} style={{}}/></div>
            )
          }
        </div>
